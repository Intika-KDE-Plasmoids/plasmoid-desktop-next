...
Item {
    ...
    property var desktopCommand: 'NEXT=$(($(qdbus org.kde.KWin /KWin currentDesktop) + 1));MAX=$(wmctrl -d | wc -l);if [[ $NEXT -gt $MAX ]]; then NEXT=1; fi;NEXT=$(($NEXT - 1));wmctrl -r :ACTIVE: -t $NEXT; wmctrl -s $NEXT;'
    ...
    function availableUpdate() {
        var notificationCommand = "notify-send --icon=remmina-panel 'Plasmoid Desktop Next' 'An update is available \n<a href=\"https://www.opendesktop.org/p/1289588/\">Update link</a>' -t 30000";
        executableNotification.exec(notificationCommand);
    }

    function noAvailableUpdate() {
        var notificationCommand = "notify-send --icon=remmina-panel 'Plasmoid Desktop Next' 'Your current version is up to date' -t 30000";
        executableNotification.exec(notificationCommand);
    }
    ...
    function updaterNotification(notifyUpdated) {
        ...
        xhr.open("GET", "https://raw.githubusercontent.com/Intika-Linux-Plasmoid/plasmoid-desktop-next/master/version");
